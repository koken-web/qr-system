...
function validateTicket(ticket: Ticket): void {
  if (
    ...
    !Number.isFinite(ticket.updatedAt)
  ) {
...
function getExpectedCounts(tickets: Ticket[]): Map<string, number> {
  ...
  for (const ticket of tickets) {
    ...
    const qrNumbers =
      qrNumbersByEvent.get(ticket.eventId) ??
      new Set<string>();

    if (qrNumbers.has(ticket.qrNumber)) {
      throw new Error(
        `同じイベント内に重複したQR番号があります: ${ticket.qrNumber}`,
      );
    }

    qrNumbers.add(ticket.qrNumber);
    qrNumbersByEvent.set(
      ticket.eventId,
      qrNumbers,
    );
    counts.set(
      ticket.eventId,
      (counts.get(ticket.eventId) ?? 0) + 1,
    );
  }
...
async function verifyTicketsStored(
  ...
  for (const [eventId, expectedCount] of expectedCounts) {
    ...
    const actualTickets = await requestToPromise(
      index.getAll(eventId),
    );

    if (actualTickets.length !== expectedCount) {
      throw new Error(
        `チケット配布確認に失敗しました。${actualTickets.length}/${expectedCount}件`,
      );
    }

    const actualQrNumbers = new Set(
      actualTickets.map(
        (ticket) => ticket.qrNumber,
      ),
    );

    if (actualQrNumbers.size !== expectedCount) {
      throw new Error(
        "チケット配布確認に失敗しました。QR番号に重複があります。",
      );
    }
  }
}
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    transaction.onabort = () =>
      reject(
        transaction.error ??
          new Error(
            "Ticket save transaction was aborted.",
          ),
      );
  });
...
export async function replaceTicketsForEvent(
  eventId: string,
  tickets: Ticket[],
): Promise<void> {
  if (!eventId) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    cursorRequest.onerror = () =>
      reject(
        cursorRequest.error ??
          new Error("Failed to replace tickets."),
      );
    ...
    transaction.onabort = () =>
      reject(
        transaction.error ??
          new Error(
            "Ticket replacement transaction was aborted.",
          ),
      );
  });

  await verifyTicketsStored(
    new Map([
      [eventId, normalizedTickets.length],
    ]),
  );
}
...
export async function getTickets(
  ...
  return requestToPromise(
    store.index("eventId").getAll(eventId),
  );
}
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ??
          new Error(
            "Failed to find tickets for deletion.",
          ),
      );
    ...
    transaction.onabort = () =>
      reject(
        transaction.error ??
          new Error(
            "Ticket delete transaction was aborted.",
          ),
      );
  });
}
